                ...
                await client.connect((err) => {
		                if (err) {
		                  ...
		                  console.log("connection established successfully");
                                  if(fileIncluded) {
                                     if(uploadFile(obj.VersicherungsNummer, obj.file, database))
...
async function uploadFile(patient, path, dbconnection) {
...
async function downloadFile(patient,path,dbconnection) {
        const collection_3 = dbconnection.collection(Diagnostik.files);
        const fileName = patient+"_"+path.split('/').pop();
        ...
        const documentPdf = await collection_3.findOne({
            ...
            const documentId = documentPdf.id;
